...
void critical_point_HISD()
{
    ...
    option2->betau = 0.02;
    option2->epsf = 1e-7;
    option2->maxiter = 10000;
    option2->outputp = 100;
    option2->outputd = 1000;

	FuncsLinear2Cplx(u_R_local, CplxDofs_local,1.0, u_R_local,0.01, option1->V[k-1]);

    copy_option_V(option1,option2,k);

    hiosd_lobpcg_sieh(u_R_local,option2);

}

void critical_point_HISD_v()
{
    int k = 1;
    hiosd_lobpcg_initialization(option2,k);
    option2->dt = 0.8;
	...
	get_initial_value(v,2,0.0);
	char filename[100];
	//sprintf(filename, "./result/%s","v_he.txt");
	//load_phi(v,filename);

	Distribute_C(v, v_local);
	my_fft(v_local,v_local);
	MPI_Gather(v_local,alloc_local,MPI_C_DOUBLE_COMPLEX,v,alloc_local,MPI_C_DOUBLE_COMPLEX,0,MPI_COMM_WORLD);
	//get_he(v);
	//Distribute_C(v, v_local);
	//sprintf(filename, "./result/%s","v_he.txt");
	//if(myid == 0)
	...
	{
		//option2->V[k-1][i][0] = v_local[i][0];
		option2->V[k-1][i][0] = option1->V[k-1][i][0];
		option2->V[k-1][i][1] = 0.0;
	}
    printf("\n hhh2 \n");
...
void hiosd_lobpcg_sieh(fftw_complex *x, Option *option)
{
    ...
    while(iter < maxiter)
    {
       ...
       // fftw_execute(p);
       // FFT_dot_constant(rhoCplx1,CplxDofs_local,1.0/CplxDofs_local);
		//if(myid == 0)
